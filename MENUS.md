...
15) Enable AS-REP roasting - uac: DONT_REQ_PREAUTH (Requires: GenericWrite on userAccountControl)
16) Force RC4 tickets - set msDS-SupportedEncryptionTypes=4 (Requires: GenericWrite on user/computer)
17) Perform RBCD attack (Requires: AllowedToAct on computer)
18) Perform RBCD attack on SPN-less user (Requires: AllowedToAct on computer & MAQ=0)
19) Perform ShadowCredentials attack (Requires: AddKeyCredentialLink)
20) Remove added ShadowCredentials (Requires: AddKeyCredentialLink)
21) Abuse GPO to execute command (Requires: GenericWrite on GPO)
22) Add Unconstrained Delegation rights - uac: TRUSTED_FOR_DELEGATION (Requires: SeEnableDelegationPrivilege rights)
23) Add DCSync rights (Requires: GenericWrite)
24) Add CIFS and HTTP SPNs entries to computer with Unconstrained Deleg rights - ServicePrincipalName & msDS-AdditionalDnsHostName (Requires: Owner of computer)
25) Add userPrincipalName to perform Kerberos impersonation of another user (Targeting Linux machines) (Requires: GenericWrite on user)
26) Modify userPrincipalName to perform Certificate impersonation (ESC10) (Requires: GenericWrite on user)
27) Add Constrained Delegation rights - uac: TRUSTED_TO_AUTH_FOR_DELEGATION (Requires: SeEnableDelegationPrivilege rights)
28) Add HOST and LDAP SPN entries of DC to computer with Constrained Deleg rights - msDS-AllowedToDelegateTo (Requires: Owner of computer)
29) Add dMSA to exploit BadSuccessor on Server 2025 (Requires: GenericWrite on OU)
30) Remove dMSA to clean after exploiting BadSuccessor (Requires: GenericWrite on OU)
31) Modify custom attribute using bloodyad (Requires: GenericWrite)
```
